...
class ExperimentResult:
    ...
    # Comparison data
    comparison_metrics: Optional[pd.DataFrame] = None
...
class ResultsManager:
    ...
    def __init__(self, base_path: str = "./results"):
        ...
        for path in [self.experiments_path, self.models_path, 
                     self.reports_path, self.visualizations_path]:
    ...
    def save_experiment_result(self, experiment: ExperimentResult) -> str:
        ...
        exp_data = {
            ...
            'config': experiment.config.to_dict() if hasattr(experiment.config, 'to_dict') else experiment.config,
            'model_results': {name: result.to_dict() for name, result in experiment.model_results.items()},
    ...
    def list_models(self, experiment_id: Optional[str] = None) -> pd.DataFrame:
    ...
    def compare_experiments(self, experiment_ids: List[str], 
                          metric: str = 'test_accuracy') -> pd.DataFrame:
        ...
        for exp_id in experiment_ids:
            ...
            if experiment:
                for model_name, model_result in experiment.model_results.items():
                    ...
                    for metric_name in ['test_accuracy', 'test_precision', 
    ...
    def export_results(self, experiment_id: str, format: str = 'csv',
...
# Utility functions
def create_model_result(model_name: str, model_type: str,
...
def aggregate_cross_validation_results(cv_results: List[ModelResult],
                                     model_name: str,
                                     model_type: str) -> ModelResult:
    ...
    metrics_to_average = ['test_accuracy', 'test_precision', 'test_recall', 
                         'test_f1', 'test_auc']
