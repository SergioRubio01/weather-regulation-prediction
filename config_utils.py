...
class ConfigurationManager:
    ...
    def create_config_wizard(self) -> ExperimentConfig:
        ...
        if use_tuning:
            ...
            tuning_method = ['grid', 'random', 'bayesian'][int(method_idx) - 1] if method_idx.isdigit() else 'grid'
    ...
    def optimize_config_for_hardware(self, config: ExperimentConfig, gpu_memory_gb: int = 8) -> ExperimentConfig:
        """
        ...
        for model_name in ['lstm', 'cnn', 'gru', 'transformer']:
            ...
            if model_config and hasattr(model_config, 'batch_size'):
                ...
                model_config.batch_size = [
                    int(bs * memory_factor) for bs in original_batch_sizes
                ]
    ...
    def generate_experiment_report(self, config: ExperimentConfig) -> str:
        ...
        report.append(f"- **Data Split:** Train {1-config.data.test_size-config.data.validation_size:.0%} / Val {config.data.validation_size:.0%} / Test {config.data.test_size:.0%}")
        ...
        report.append(f"- **Cross-validation:** {'Yes' if config.training.use_cross_validation else 'No'} ({config.training.cv_folds} folds)")
        report.append(f"- **Early stopping:** {'Yes' if config.training.use_early_stopping else 'No'} (patience: {config.training.early_stopping_patience})")
        report.append(f"- **Hardware:** GPU {'enabled' if config.training.use_gpu else 'disabled'}, Mixed precision {'on' if config.training.mixed_precision else 'off'}")
