...
class TransformerModel(BaseModel):
    ...
    def build_model(
        ...
        **kwargs,
        ...
        use_positional_encoding = (
            use_positional_encoding
            if use_positional_encoding is not None
            else self.model_config.use_positional_encoding
        )
    ...
    def _tune_hyperparameters(
        ...
        tuner.search(
            ...
            validation_data=validation_data,
            callbacks=(
                [EarlyStopping(patience=10)] if self.config.training.use_early_stopping else []
            ),
            verbose=1,
        )
    ...
    def get_attention_weights(self, X: np.ndarray) -> dict[str, np.ndarray]:
        ...
        for i, layer in enumerate(self.model.layers):
            if "transformer_block" in layer.name:
                ...
                attention_model = Model(inputs=self.model.input, outputs=layer.attention.output)
