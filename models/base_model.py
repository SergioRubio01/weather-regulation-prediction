...
class BaseModel(ABC):
    ...
    def evaluate(
        self, X_test: np.ndarray, y_test: np.ndarray, save_plots: bool = True
    ) -> ModelMetrics:
        """
    ...
    def plot_metrics(self):
        ...
        for i, (_, v) in enumerate(metrics_dict.items()):
    ...
    def save_results(self, prefix: str = ""):
        ...
        if self.best_params:
            params_path = (
                self.output_dir / f"{prefix}{self.model_type}_best_params_{timestamp}.json"
            )
