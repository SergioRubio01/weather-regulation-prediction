...
class EnsembleModel(BaseModel):
    ...
    def _initialize_base_models(self):
        ...
        for model_name in self.model_config.base_models:
            if model_name in model_classes:
                ...
                if not TF_AVAILABLE and model_name in ["lstm", "cnn", "gru", "fnn"]:
                    self.logger.warning(f"Skipping {model_name} - TensorFlow not available")
                else:
    ...
    def _build_voting_model(self):
        ...
        self.model = VotingClassifier(
            estimators=estimators, voting=self.model_config.voting_type, weights=weights, n_jobs=-1
        )
    ...
    def _train_base_models_individually(
        ...
        for model_name, model in self.base_models.items():
            ...
            if X_val is not None and y_val is not None:
                ...
                self.logger.info(f"{model_name} validation accuracy: {metrics.accuracy:.4f}")
    ...
    def _predict_voting(self, X: np.ndarray) -> np.ndarray:
        ...
        for i, (_, model) in enumerate(self.base_models.items()):
