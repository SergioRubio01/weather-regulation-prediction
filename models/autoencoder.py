...
from config import ExperimentConfig, AutoencoderConfig
...
class AutoencoderModel(BaseModel):
    ...
    def train(
        ...
        **kwargs
        ...
        if self.model_config.use_for_feature_extraction:
            ...
            if self.model_config.fine_tune_classifier and not self.model_config.fine_tune_classifier:
                ...
                # Fine-tune
                ft_history = self.classifier.fit(
                    X_train, y_train,
                    validation_data=validation_data,
                    epochs=epochs // 4,
                    batch_size=batch_size,
    ...
    def detect_anomalies(self, X: np.ndarray, threshold: Optional[float] = None) -> np.ndarray:
        """
