...
from config import ExperimentConfig, CNNConfig
...
class CNNModel(BaseModel):
    ...
    def build_model(
        ...
        **kwargs
        ...
        use_batch_norm = use_batch_norm if use_batch_norm is not None else self.model_config.use_batch_norm
    ...
    def get_feature_maps(self, X: np.ndarray, layer_index: int = 0) -> np.ndarray:
        ...
        if layer_index >= len(conv_layers):
            raise ValueError(f"Layer index {layer_index} out of range. Model has {len(conv_layers)} conv layers.")
