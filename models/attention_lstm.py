...
from keras.layers import (
    Input, LSTM, Dense, Dropout, Layer, Concatenate,
    Permute, Reshape, Multiply, Lambda, RepeatVector,
...
class AttentionLSTMModel(BaseModel):
    ...
    def _bahdanau_attention(self, lstm_output, units):
        ...
        # Compute scores
        scores = score_second_layer(keras.activations.tanh(
            score_first_layer(lstm_output)
        ))
    ...
    def _save_model_implementation(self, filepath: Path):
        ...
        self.attention_model.save(
            str(filepath.with_suffix('_attention.h5')), 
            save_traces=False
        )
