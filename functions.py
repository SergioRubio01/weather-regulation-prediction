...
from PythonMETAR import *  # METAR decoder
from metar_taf_parser.parser.parser import TAFParser  # TAF decoder
from ast import literal_eval
from metafora import Taf, unify_forecasts  # TAF decoder
...
from keras.layers import Conv1D, MaxPooling1D, Flatten, Input, Activation, Add
...
def df_filter(df,filter_column,filter):
    ...
    if filter_column in df:
        ...
        print(f'Error. No column exists in the table')
    return df_filtered
...
def filter_METARs(data_path, data_path_out,time_step,download_type):
    ...
    if download_type == 1:
        ...
        metar_data_init['Date'] = pd.to_datetime(metar_data_init[['year', 'month', 'day', 'hour', 'minute']])
        ...
        for i in range(0,len(times)-1):
            ...
            if abs(time_difference/60) > time_step:
                ...
                for _ in range(multiples - 1):
                    ...
                    new_metar_report.append(metar_data['Report'].iloc[i])
                    new_metar_date.append((metar_data['Date'].iloc[i] + dt.timedelta(minutes=time_step)).strftime("%Y-%m-%d %H:%M:%S"))
            else:
        ...
        metar_data = pd.DataFrame(
            {'Date': new_metar_date, 'Report': new_metar_report})
        ...
        for i in range(len(m_winds)):
            ...
            if (extra_var1 != None):
                ...
                metar_temp_info.extend(
                    [temperature, extra_var1.get('dewpoint')])
            else:
        ...
        for i in range(len(metar_data)):
            data_metar = {'Complete date':  metar_data['Date'],
...
def filter_by_dates(regulation_file, metar_file):
    ...
    filtered_metar_data = metar_data[metar_data['Complete date'].isin(regulation_data['Initial Time'])]
...
def reg_binarizer(time_init, time_end, time_delta, reg_path, out_path):
    ...
    reg_path['Regulation Start Time'] = pd.to_datetime(
        reg_path['Regulation Start Time'])
    reg_path['Regulation End Date'] = pd.to_datetime(
        reg_path['Regulation End Date'])
    ...
    while time_init <= time_end:
        ...
        for i in range(0,len(reg_path)):
            if (time_upper > start_time_lista[i] and start_time_lista[i] >= time_lower) or (start_time_lista[i] <= time_lower < end_time_lista[i]):
...
def Reg_binarizer_undersampling(file_paths, ADs, output_file):
    ...
    final_df = pd.DataFrame({'Initial Time': sorted(list(all_dates))})
    for airport in ADs:
        final_df[airport] = 0
...
def Reg_binarizer_oversampling(Input_file, Output_file):
    ...
    for i in range(1, len(df)):
        if (df.loc[i, 'Regulation Binary'] == 0 and df.loc[i - 1, 'Regulation Binary'] == 1) or (df.loc[i, 'Regulation Binary'] == 1 and df.loc[i - 1, 'Regulation Binary'] == 0):
            if not change_flag:
...
def plot_roc_curve(X, y, n_trees, criterion, AD, test_size, max_depth, random_state, Output_path):
    ...
    plt.title(
        f'ROC Curve - Random Forest ({n_trees} trees, {criterion} criterion)')
...
def graphs_weather(input_file, AD):
    ...
    weather_matrix = all_reg_matrix[all_reg_matrix['Regulation Reason Name']
                                    == 'W - Weather']
    ...
    weather_airport_type = airport_matrix['Regulation Description'].str.lower(
    ).tolist()
    ...
    for column in range(len(weather_type)):
        if not pd.isna(weather_type[column]):
            weather_type[column] = 'CB' if any(keyword in weather_type[column] for keyword in ['cb', 'rain', 'show', 'thunderstorm']) else \
                                   'VISIBILITY/CEILING' if any(keyword in weather_type[column] for keyword in ['visibility', 'ceiling', 'vis', 'lvp', 'l.v.p.']) else \
                                   'WIND' if 'wind' in weather_type[column] else \
                                   'FOG' if 'fog' in weather_type[column] else \
                                   'SNOW/ICE' if any(keyword in weather_type[column] for keyword in ['snow', 'freez', 'icing', 'ice']) else \
                                   'OTHER'
        else:
    ...
    for column in range(len(weather_airport_type)):
        if not pd.isna(weather_airport_type[column]):
            weather_airport_type[column] = 'CB' if any(keyword in weather_airport_type[column] for keyword in ['cb', 'rain', 'show', 'thunderstorm']) else \
                'VISIBILITY/CEILING' if any(keyword in weather_airport_type[column] for keyword in ['visibility', 'ceiling', 'vis', 'lvp', 'l.v.p.']) else \
                'WIND' if 'wind' in weather_airport_type[column] else \
                'FOG' if 'fog' in weather_airport_type[column] else \
                'SNOW/ICE' if any(keyword in weather_airport_type[column] for keyword in ['snow', 'freez', 'icing', 'ice']) else \
                'OTHER'
        else:
    ...
    plt.xlabel('')
    plt.ylabel('Percentage of counts')
    plt.title(f'Regulations types in Eurocontrol area')
    # Rotamos las etiquetas del eje X para mayor legibilidad
    plt.xticks(rotation=60)
    plt.tight_layout()
    ...
    sorted_counts1, sorted_words1 = zip(
        *sorted(zip(counts1, words1), reverse=True))
...
def labelbinarizer_WD_preprocess(file_path, input_column, output_columns, Output_file):
    ...
    data['Wind Direction'] = pd.cut(
        data[input_column], bins=intervals, labels=labels)

    # Realiza la codificación one-hot para obtener las columnas de salida
    binarized_df = pd.get_dummies(data['Wind Direction'], columns=[
                                  'Wind Direction'], prefix=output_columns)
...
def labelbinarizer_CC_preprocess(file_path, output_columns,Output_file):
    ...
    # Realiza la codificación one-hot para obtener las columnas de salida
    binarized_df = pd.get_dummies(data['Ceiling coverage'], columns=[
                                  'Ceiling coverage'], prefix=output_columns)
...
###################################################################################
###################################################################################
def LSTM_model_1(X, Y, epochs, batch_size, AD, test_size, random_state, optimizer, activation, loss, dropout_rate, units, Output_path):
    """
...
###################################################################################
###################################################################################
def LSTM_model_2(X, Y, epochs, batch_size, AD, test_size, random_state, optimizer, activation, loss, Output_path):
    """
    This function is intended to create a LSTM model for computing a binary classifier
    in the ATFM weather-related regulations problem
...
###################################################################################
###################################################################################
def RNN_model(X, Y, epochs, batch_size, AD, test_size, random_state, optimizer,activation,loss,Output_path):
    # Dividir los datos en conjuntos de entrenamiento y prueba
    X_train, X_test, Y_train, Y_test = separar(
        X, Y, test_size=test_size, random_state=random_state)
    X_train, X_val, Y_train, Y_val = separar(
    ...
    # Crear el modelo RNN
    model = Sequential()
    model.add(SimpleRNN(epochs, input_shape=(
...
###################################################################################
###################################################################################
def CNN_RNN_model(X, Y, epochs, batch_size, AD, time_init, time_end, test_size, random_state, activationDense, activationConv1D, optimizer, loss, kernel_size, pool_size, Conv1D_layer, Dense_layer, Output_path):
    # Dividir los datos en conjuntos de entrenamiento y prueba
    X_train, X_test, Y_train, Y_test = separar(
        X, Y, test_size=test_size, random_state=random_state)
    ...
    cnn_model.add(Conv1D(Conv1D_layer, kernel_size=kernel_size, activation=activationConv1D,
                  input_shape=(X_train.shape[1], 1)))
    ...
    rnn_model.add(SimpleRNN(epochs, input_shape=(
        X_train.shape[1], 1), return_sequences=True))
...
###################################################################################
###################################################################################
def CNN_model(X, Y, epochs, batch_size, AD, time_init, time_end, test_size, random_state, activationDense, activationConv1D, optimizer, loss, kernel_size, pool_size, Conv1D_layer, Dense_layer, Output_path):
    # Dividir los datos en conjuntos de entrenamiento y prueba
    X_train, X_test, Y_train, Y_test = separar(
        X, Y, test_size=test_size, random_state=random_state)
    ...
    # Crear el modelo CNN
    model = Sequential()
    model.add(Conv1D(Conv1D_layer, kernel_size=kernel_size,
              activation=activationConv1D, input_shape=(X_train.shape[1], 1)))
...
###################################################################################
###################################################################################
def CNN_LSTM_model(X, Y, epochs, batch_size, AD, time_init, time_end, test_size, random_state, activationDense, activationConv1D, optimizer, loss, kernel_size, pool_size, Conv1D_layer, Dense_layer, Output_path):
    # Dividir los datos en conjuntos de entrenamiento y prueba
    X_train, X_test, Y_train, Y_test = separar(
        X, Y, test_size=test_size, random_state=random_state)
    ...
    X_train_reshaped = X_train.reshape(
        X_train.shape[0], timesteps, X_train.shape[1])
    X_test_reshaped = X_test.reshape(
        X_test.shape[0], timesteps, X_test.shape[1])
    ...
    lstm_model.add(LSTM(epochs, input_shape=(
    ...
    # Entrenar el modelo
    history = combined_model.fit(
        X_train_reshaped, Y_train, epochs=epochs, batch_size=batch_size, validation_data=(X_test_reshaped, Y_test))
...
###################################################################################
###################################################################################
def WaveNet_model(X, Y, epochs, batch_size, AD, test_size, random_state, optimizer, activation, loss, Output_path):
    """
    ...
    residual = Conv1D(filters=128, kernel_size=2,
                      padding='causal')(input_layer)
    skip_connections = []
    for _ in range(5):  # Adjust the number of layers as needed
        dilated_conv = Conv1D(filters=128, kernel_size=2, padding='causal',
                              activation='relu', dilation_rate=2)(residual)
