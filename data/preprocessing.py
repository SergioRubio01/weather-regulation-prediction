...
class TimeSeriesScaler(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, method: str = "standard", window_size: int | None = None):
        self.method = method
    ...
    def inverse_transform(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.window_size:
            ...
            for i in range(n_windows):
                ...
                if i in self.scalers_:
                    X_original[start_idx:end_idx] = self.scalers_[i].inverse_transform(
                        X[start_idx:end_idx]
                    )
                else:
                    last_scaler = self.scalers_[max(self.scalers_.keys())]
                    X_original[start_idx:end_idx] = last_scaler.inverse_transform(
                        X[start_idx:end_idx]
                    )
            ...
            if feature in X.columns:
                ...
                for window in self.rolling_windows:
                    for func in self.rolling_funcs:
                        if func == "mean":
                            X_transformed[f"{feature}_rolling_{func}_{window}"] = (
                                X[feature].rolling(window).mean()
                            )
                        elif func == "std":
                            X_transformed[f"{feature}_rolling_{func}_{window}"] = (
                                X[feature].rolling(window).std()
                            )
                        elif func == "min":
                            X_transformed[f"{feature}_rolling_{func}_{window}"] = (
                                X[feature].rolling(window).min()
                            )
                        elif func == "max":
                            X_transformed[f"{feature}_rolling_{func}_{window}"] = (
                                X[feature].rolling(window).max()
                            )
                        elif func == "median":
                            X_transformed[f"{feature}_rolling_{func}_{window}"] = (
                                X[feature].rolling(window).median()
                            )
...
class OutlierDetector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None) -> "OutlierDetector":
        ...
        if self.method == "iqr":
            ...
            self.detector_ = LocalOutlierFactor(contamination=self.contamination, novelty=True)
            self.detector_.fit(X)
                ...
                for col in X.columns:
                    if col in self.bounds_["lower"].index:
                        X_transformed[col] = X_transformed[col].clip(
                            lower=self.bounds_["lower"][col], upper=self.bounds_["upper"][col]
                        )
        ...
        if self.method == "mutual_info":
            ...
            if isinstance(self.n_features, int):
                ...
                self.selector_ = SelectPercentile(
                    score_func=score_func, percentile=int(self.n_features * 100)
                )
            ...
            if isinstance(self.n_features, int):
                ...
                self.selector_ = SelectPercentile(
                    score_func=chi2, percentile=int(self.n_features * 100)
                )
            ...
            self.selector_ = RFE(estimator=estimator, n_features_to_select=n_features_to_select)
            ...
            if self.task == "classification":
                estimator = LogisticRegressionCV(
                    cv=self.cv, penalty="l1", solver="liblinear", random_state=42
                )
            else:
            ...
            self.selector_ = SelectFromModel(estimator=estimator, threshold="median")
...
class PreprocessingPipeline:
    ...
    def __init__(self, config: DataConfig | ExperimentConfig):
    ...
    def create_pipeline(
        ...
        cyclical_features: dict[str, int] | None = None,
        lag_features: dict[str, list[int]] | None = None,
        ...
        if self.config.outlier_method:
            ...
            outlier_detector = OutlierDetector(
                method=self.config.outlier_method, threshold=self.config.outlier_threshold
            )
        ...
        if lag_features:
            ...
            for feature, lags in lag_features.items():
                lag_creator = LagFeatureCreator(
                    features=[feature], lags=lags, rolling_windows=[3, 6, 12, 24]
                )
        ...
        if categorical_features:
            ...
            if low_cardinality:
                encoders.append(
                    ("onehot", OneHotEncoder(drop="first", sparse=False), low_cardinality)
                )
        ...
        if self.config.feature_transform:
            ...
            if self.config.feature_transform == "polynomial":
                ...
                log_transformer = FunctionTransformer(
                    func=lambda X: np.log1p(np.abs(X)), validate=False
                )
...
def preprocess_weather_data(
    data: pd.DataFrame, config: str | ExperimentConfig, target_column: str = "has_regulation"
    ...
    cyclical_features = {"hour": 24, "day_of_week": 7, "month": 12}
