...
class SchemaValidator(BaseValidator):
    ...
    def _validate_constraints(self, series: pd.Series, constraints: Dict[str, Any],
                            col_name: str, result: ValidationResult) -> None:
        ...
        if 'values' in constraints:
            ...
            if invalid_values.any():
                ...
                result.warnings.append(
                    f"Column '{col_name}' has invalid values: {unique_invalid}"
                )
...
class DataQualityValidator(BaseValidator):
    ...
    def _check_missing_values(self, data: pd.DataFrame, result: ValidationResult) -> None:
        ...
        if 'timestamp' in data.columns:
            ...
            missing_by_hour = data.set_index('timestamp').resample('H').apply(
    ...
    def _check_consistency(self, data: pd.DataFrame, result: ValidationResult) -> None:
        ...
        if 'timestamp' in data.columns:
            ...
            if large_gaps.any():
                result.warnings.append(
                    f"Found {large_gaps.sum()} time gaps larger than 24 hours"
                )
    ...
    def _check_completeness(self, data: pd.DataFrame, result: ValidationResult) -> None:
        ...
        if 'timestamp' in data.columns:
            ...
            if completeness_ratio < 0.8:
                result.warnings.append(
                    f"Low temporal completeness: {completeness_ratio:.2%}"
                )
    ...
    def _generate_recommendations(self, result: ValidationResult) -> None:
        ...
        if 'anomaly_counts' in result.statistics:
            result.recommendations.append(
                "Apply outlier detection and treatment before modeling"
            )
...
class AnomalyDetector:
    ...
    def __init__(self, contamination: float = 0.01,
                 methods: List[str] = ['isolation_forest', 'elliptic_envelope']):
...
class DataDriftDetector:
    ...
    def __init__(self, statistical_tests: List[str] = ['ks', 'chi2'],
            ...
            if col in self.reference_stats['distributions']:
                ...
                if 'chi2' in self.statistical_tests:
                    ...
                    curr_hist, _ = np.histogram(
                        X_current[col].dropna().values, bins=bins
                    )
                    ...
                    statistic, p_value = stats.chisquare(
                        curr_hist + 1e-10,  # Avoid division by zero
                        ref_hist + 1e-10
                    )
                ...
                # Check for drift
                is_drifted = any(
                    result['p_value'] < self.drift_threshold
    ...
    def visualize_drift(self, X_reference: pd.DataFrame, X_current: pd.DataFrame,
        ...
        for idx, feature in enumerate(features[:6]):
            ...
            # Plot distributions
            ax.hist(X_reference[feature].dropna(), bins=30, alpha=0.5, 
                   label='Reference', density=True)
            ax.hist(X_current[feature].dropna(), bins=30, alpha=0.5,
...
class DataValidator:
    ...
    def validate_dataset(self, data: pd.DataFrame, 
                        ...
                        reference_data: Optional[pd.DataFrame] = None) -> Dict[str, ValidationResult]:
        ...
        if reference_data is not None:
            ...
            drift_result = ValidationResult(
                is_valid=not drift_results['overall_drift'],
                statistics=drift_results
            )
...
def validate_weather_data(data: pd.DataFrame) -> ValidationResult:
    ...
    schema = DataSchema(
        ...
        numerical_columns=['temperature', 'dewpoint', 'pressure', 'wind_speed',
                         'wind_direction', 'visibility', 'ceiling']
    )
