...
try:
    ...
    warnings.warn("Weather parsing libraries not available. Install pytaf, metar-taf-parser, and metafora.")
...
class DatasetInfo:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'time_range': [t.isoformat() if t else None for t in self.time_range] if self.time_range else None,
...
class DataCache:
    ...
    def get(self, data_path: str, params: Dict[str, Any]) -> Optional[pd.DataFrame]:
...
class DataLoader:
    ...
    def __init__(self, config: Union[DataConfig, ExperimentConfig], 
        ...
        # Loaded data
        self.metar_data: Optional[pd.DataFrame] = None
        self.taf_data: Optional[pd.DataFrame] = None
        self.regulations_data: Optional[pd.DataFrame] = None
    ...
    def load_all_data(self, airports: List[str], 
    ...
    def load_metar_data(self, airports: List[str], 
        ...
        for airport in airports:
            ...
            files = self._find_files_in_range(airport_path, start_date, end_date, 
                                            pattern="*.csv")
            
            if self.parallel and len(files) > 1:
                # Parallel loading
                with ThreadPoolExecutor(max_workers=4) as executor:
                    futures = [executor.submit(self._load_metar_file, f, airport) 
                             for f in files]
        ...
        if metar_dfs:
            ...
            metar_data = metar_data[
                (metar_data['datetime'] >= start_date) & 
                (metar_data['datetime'] <= end_date)
            ]
    ...
    def _load_metar_file(self, file_path: Path, airport: str) -> Optional[pd.DataFrame]:
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Parsing METAR"):
            try:
                ...
                if parsed:
                    ...
                    parsed['datetime'] = pd.to_datetime(row.get('observation_time', 
                                                              row.get('datetime')))
        ...
        if not parsed:
            try:
                ...
                if metar_obj.wind:
                    ...
                    parsed['wind_direction'] = metar_obj.wind.get('direction', {}).get('value', np.nan)
                ...
                if metar_obj.clouds:
                    # Get lowest ceiling
                    ceilings = [c.get('height', {}).get('value', float('inf')) 
    ...
    def load_taf_data(self, airports: List[str], 
                     start_date: pd.Timestamp, 
                     end_date: pd.Timestamp) -> pd.DataFrame:
        ...
        if taf_dfs:
            ...
            # Filter date range
            taf_data = taf_data[
                (taf_data['valid_from'] >= start_date) & 
                (taf_data['valid_from'] <= end_date)
            ]
    ...
    def _load_taf_file(self, file_path: Path, airport: str) -> Optional[pd.DataFrame]:
    ...
    def load_regulation_data(self, airports: List[str], 
                           start_date: pd.Timestamp, 
                           end_date: pd.Timestamp) -> pd.DataFrame:
        ...
        if 'reason' in regulations.columns:
            ...
            weather_keywords = ['WEATHER', 'WX', 'CB', 'TS', 'FOG', 'WIND', 'SNOW', 
                              'ICE', 'VISIBILITY', 'CEILING', 'STORM']
            
            regulations['is_weather_related'] = regulations['reason'].str.upper().str.contains(
                '|'.join(weather_keywords), na=False
            )
        else:
    ...
    def _resample_metar_data(self, metar_df: pd.DataFrame, 
        ...
        # Resample
        resampled = metar_df[existing_cols].resample(
            f'{self.config.time_step_minutes}T'
    ...
    def _create_taf_features(self, taf_df: pd.DataFrame, 
                           time_index: pd.DatetimeIndex) -> pd.DataFrame:
        ...
        for col in ['wind_speed_kt', 'wind_direction', 'visibility_m', 'ceiling_ft']:
            if col in taf_df.columns:
                ...
                for idx, row in taf_df.iterrows():
    ...
    def _create_regulation_labels(self, reg_df: pd.DataFrame, 
                                time_index: pd.DatetimeIndex) -> pd.DataFrame:
        ...
        for idx, reg in reg_df.iterrows():
            ...
            if 'severity' in reg:
                labels.loc[mask, 'regulation_severity'] = max(
                    labels.loc[mask, 'regulation_severity'],
                    reg['severity']
                )
    ...
    def _calculate_relative_humidity(self, temp_c: pd.Series, 
                                   dewpoint_c: pd.Series) -> pd.Series:
    ...
    def _calculate_wind_chill(self, temp_c: pd.Series, 
                            wind_speed_kt: pd.Series) -> pd.Series:
        """Calculate wind chill index"""
        # Convert knots to km/h
        wind_kmh = wind_speed_kt * 1.852
        
        # Wind chill formula (valid for temp <= 10°C and wind >= 4.8 km/h)
        wind_chill = 13.12 + 0.6215 * temp_c - 11.37 * (wind_kmh ** 0.16) + \
    ...
    def _handle_missing_values(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df[numeric_cols] = df[numeric_cols].fillna(method='ffill', limit=2).fillna(method='bfill', limit=2)
    ...
    def _find_files_in_range(self, directory: Path, start_date: pd.Timestamp,
                           end_date: pd.Timestamp, pattern: str = "*") -> List[Path]:
...
def load_weather_data(config: Union[str, ExperimentConfig], 
                     airports: List[str],
...
def create_feature_matrix(config: Union[str, ExperimentConfig],
                         airports: List[str],
    ...
    data = loader.load_all_data(airports)
