...
class ModelCheckpointer:
    ...
    def save_sklearn_model(self, model: Any, metrics: dict[str, float]) -> str:
        ...
        checkpoint_data = {"model": model, "metrics": metrics, "timestamp": timestamp}
...
class Trainer:
    ...
    def __init__(
        self, config: ExperimentConfig, experiment_tracker: ExperimentTracker | None = None
    ):
        self.config = config
        self.experiment_tracker = experiment_tracker
        self.logger = self._setup_logger()
        self.checkpointer = ModelCheckpointer(
            checkpoint_dir=config.output_dir or "./checkpoints", model_name=config.experiment_name
        )
        ...
        try:
            ...
            if self.experiment_tracker:
                self.experiment_tracker.log_model(
                    model, artifact_path="model", model_type=model_type
                )
    ...
    def _train_keras_model(
        ...
        X_val: np.ndarray | None = None,
        y_val: np.ndarray | None = None,
        custom_callbacks: list[TrainingCallback] | None = None,
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "model": model,
            "checkpoint_path": str(
                self.checkpointer.checkpoint_dir / f"{self.config.experiment_name}_best.h5"
            ),
        }
    ...
    def _get_keras_callbacks(self) -> list:
        ...
        if self.config.output_dir:
            log_dir = (
                Path(self.config.output_dir) / "logs" / datetime.now().strftime("%Y%m%d_%H%M%S")
            )
            callbacks.append(
                TensorBoard(
                    log_dir=str(log_dir), histogram_freq=1, write_graph=True, update_freq="epoch"
                )
            )
    ...
    def cross_validate(
        self, model_class: Any, X: np.ndarray, y: np.ndarray, cv_folds: int = 5, **model_kwargs
    ) -> dict[str, Any]:
        """
        ...
        cv_results = {"accuracy": [], "precision": [], "recall": [], "f1": [], "fold_models": []}
    ...
    def train_ensemble(
        ...
        results = {"models": {}, "metrics": {}, "training_time": {}}
    ...
    def _evaluate_model(
        self, model: Any, X: np.ndarray, y: np.ndarray, model_type: str
    ) -> dict[str, float]:
...
def create_trainer(
    config: str | dict | ExperimentConfig,
    ...
    if enable_tracking:
        tracker = ExperimentTracker(
            experiment_name=config.experiment_name, tracking_uri=config.output_dir
        )
