...
from config_utils import compare_configs, optimize_config_for_hardware
from training.trainer import Trainer, create_trainer
from training.hyperparameter_tuning import create_tuner, TuningResult
...
from models.hybrid_models import CNNRNNModel, CNNLSTMModel
...
class ExperimentSuite:
    ...
    def __post_init__(self):
        """Initialize suite"""
        self.output_path = Path(self.output_dir) / self.name / datetime.now().strftime("%Y%m%d_%H%M%S")
...
class ExperimentRunner:
    ...
    def run_single_experiment(self, config: ExperimentConfig, 
                            ...
                            tune_hyperparameters: bool = True,
                            tuning_method: str = 'bayesian',
                            tuning_trials: int = 50) -> ExperimentResult:
        """
        ...
        try:
            ...
            if tune_hyperparameters and config.model_type != ModelType.ENSEMBLE:
                ...
                tuner = create_tuner(
                    ...
                    model_class=lambda **params: self._create_model_instance(model_class, config, params),
            ...
            if hasattr(model, 'evaluate'):
                ...
                if len(np.unique(y_test)) == 2:
                    proba = model.predict_proba(X_test)[:, 1] if hasattr(model, 'predict_proba') else predictions
    ...
    def run_experiment_suite(self, suite: ExperimentSuite,
                           X_train: np.ndarray, y_train: np.ndarray,
                           X_test: np.ndarray, y_test: np.ndarray) -> List[ExperimentResult]:
        """
    ...
    def create_ensemble_from_results(self, results: List[ExperimentResult],
                                   ensemble_config: ExperimentConfig,
                                   X_train: np.ndarray, y_train: np.ndarray,
                                   X_test: np.ndarray, y_test: np.ndarray) -> ExperimentResult:
        """
        ...
        successful_results = [r for r in results if r.error_message is None and r.model_path is not None]
        ...
        ensemble_result = ExperimentResult(
            config_name="ensemble_from_results",
            model_type="ensemble",
            metrics={}
        )
    ...
    def compare_experiments(self, results: List[ExperimentResult],
    ...
    def generate_report(self, suite: ExperimentSuite, results: List[ExperimentResult]) -> str:
        ...
        report_html = template.render(
            ...
            plots=plots,
            experiments=[(name, yaml.dump(config.__dict__, default_flow_style=False)) 
                        for name, config in suite.experiments]
        )
    ...
    def _create_model_instance(self, model_class: Any, config: ExperimentConfig,
    ...
    def _save_model(self, model: Any, config: ExperimentConfig,
        ...
        filename = f"{config.experiment_name}_{timestamp}.pkl"
...
def create_experiment_suite_from_yaml(yaml_path: str) -> ExperimentSuite:
    ...
    for exp_config in suite_config['experiments']:
        if 'config_file' in exp_config:
            ...
            name = exp_config.get('name', config.experiment_name)
        else:
...
def run_quick_experiment(model_type: str = 'random_forest'):
    ...
    config.experiment_name = f"quick_{model_type}_experiment"
...
def run_model_comparison(output_dir: str = "./model_comparison"):
    ...
    for model_type in ModelType:
        ...
        config = parser.create_default_config()
        config.experiment_name = f"{model_type.value}_comparison"
    ...
    if len(results) > 2:
        ...
        ensemble_config.experiment_name = "ensemble_from_comparison"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='random_forest', help="Model type for quick experiment")
