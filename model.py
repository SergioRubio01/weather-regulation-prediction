...
from functions import (
    df_filter, airports_NMOC_fun, download_METARs, download_TAFs,
    filter_METARs, filter_TAFs, reg_binarizer,
    labelbinarizer_WD_preprocess, labelbinarizer_CC_preprocess,
    minmaxscaler_preprocess, filter_by_dates,
    RF_model, graphs_weather, graphs_TAF,
    RNN_model, FNN_model, WaveNet_model,
    LSTM_model_1, LSTM_model_2,
    CNN_RNN_model, CNN_LSTM_model, CNN_model,
    plot_roc_curve
)
...
from data.data_loader import DataLoader, load_weather_data
from data.data_validation import DataValidator, validate_weather_data
from data.feature_engineering import (
    AutomatedFeatureEngineer, WeatherFeatureEngineer,
    TimeSeriesFeatureEngineer, engineer_features_for_model
)
from data.preprocessing import (
    PreprocessingPipeline, create_preprocessing_pipeline,
    preprocess_weather_data
)

# Model imports
from models import (
    RandomForestModel, LSTMModel, CNNModel, RNNModel,
    FNNModel, WaveNetModel, CNNRNNModel, CNNLSTMModel,
    GRUModel, TransformerModel, AttentionLSTMModel,
    EnsembleModel, AutoencoderModel
)

# Training imports
from training.trainer import Trainer, ExperimentTracker
from training.hyperparameter_tuning import create_tuner
from run_experiments import ExperimentRunner, MODEL_REGISTRY
...
class Dly_Classifier(object):
    ...
    Supported Models:
        ...
        - And more...
    """
    def __init__(self, time_init, time_end, info_path, Output_path, AD, time_delta, download_type,
                 config: Optional[Union[str, ExperimentConfig]] = None,
    ...
    def _load_data_new_pipeline(self):
        ...
        self.y = self.features['has_regulation'].values if 'has_regulation' in self.features.columns else None
    ...
    def _load_data_legacy_pipeline(self):
        ...
        x = pd.read_csv(os.path.join(
            self.info_path, 'METAR', f'METAR_{self.AD}_filtered_final.csv'))
        self.X = x.iloc[:, 1:15].values
        
        # Compute target vector
        y = pd.read_csv(os.path.join(
            self.info_path, 'Regulations', 'Regulation_binarizer_2_30.csv'))
        self.y = y[f'{self.AD}'].values



    def train_models(self, models: Optional[List[str]] = None,
    ...
    def _train_models_new_pipeline(self, models: Optional[List[str]] = None,
                                  use_hyperparameter_tuning: bool = True) -> Dict[str, Any]:
        ...
        if self.X is None and self.features is not None:
            ...
            categorical_features = self.features.select_dtypes(include=['object', 'category']).columns.tolist()
            ...
            # Fit and transform
            X_df = self.features.drop(columns=['has_regulation'] if 'has_regulation' in self.features.columns else [])
        ...
        # Generate report
        report_path = self.Output_path / f"experiment_report_{self.AD}_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
    ...
    def _train_models_legacy_pipeline(self) -> Dict[str, Any]:
        ...
        df_results.to_csv(
            self.Output_path / f'FNN_{self.AD}_results.csv', 
            index=False
        )
    ...
    def run_modular_experiment(self, models: Optional[List[str]] = None,
        ...
        if self.config.data.feature_engineering:
            ...
            self.features = self.feature_engineer.fit_transform(
                self.features, 
                self.y
            )
    ...
    def filtered_df(self,option,time_delta=False, download_type=False):
        ...
        if option == 1:
            ...
            if download_type == 1:
                ...
                _, _, self.df_MET = minmaxscaler_preprocess(os.path.join(self.info_path, 'METAR', f'METAR_{self.AD}_filtered_dates.csv'),
                                                            ['Horizontal visibility', 'Ceiling height',
                                                                'Wind Speed', 'Temperature', 'Dewpoint'],
                ...
                self.df_MET_WD = labelbinarizer_WD_preprocess(os.path.join(self.info_path, 'METAR', f'METAR_{self.AD}_filtered_dates.csv'), 
                                                           'Wind Direction',
                                                           'WD',
                                                              os.path.join('.\Data\METAR', f'METAR_{self.AD}_WDCat.csv'))
                ...
                self.df_MET_CC = labelbinarizer_CC_preprocess(
                    ...
                    'CCCat',
                    os.path.join('.\Data\METAR', f'METAR_{self.AD}_CCCat.csv'))
                
                # Obtain final dataframe
                file1 = pd.read_csv(os.path.join('.\Data\METAR', f'METAR_{self.AD}_CCCat.csv'))
                file2 = pd.read_csv(os.path.join('.\Data\METAR', f'METAR_{self.AD}_WDCat.csv'))
                file3 = pd.read_csv(os.path.join(self.info_path, 'METAR', f'METAR_{self.AD}_filtered_minmax.csv'))
                self.df_MET_final = pd.concat(
                    [file3,file1,file2], axis=1, ignore_index=True)
                self.df_MET_final.to_csv(os.path.join(
                    '.\Data\METAR', f'METAR_{self.AD}_filtered_final.csv'), index=False)
                
                

                return self.df_MET_final
            else:
                print(f'Error (download type={download_type}). Select: 0=download from web, 1=download txt')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Weather-based ATFM Regulation Prediction'
    )
    parser.add_argument(
        '--airport', '-a',
        type=str,
        default='EGLL',
        help='Airport ICAO code (default: EGLL)'
    )
    parser.add_argument(
        '--config', '-c',
        type=str,
    ...
    parser.add_argument(
        '--models', '-m',
        nargs='+',
        help='Models to train (e.g., lstm transformer ensemble)'
    )
    parser.add_argument(
        '--no-tuning',
        action='store_true',
    ...
    if args.pipeline == 'legacy' and not args.config:
        ...
        target = Dly_Classifier(time_init, time_end, Data_path, Output_path, 
                               aerodrome, time_sep, download_type,
