...
from data.data_loader import DataLoader, DataCache
from data.data_validation import (
    DataValidator, SchemaValidator, DataQualityValidator,
    AnomalyDetector, DataDriftDetector
)
from data.feature_engineering import (
    WeatherFeatureEngineer, TimeSeriesFeatureEngineer,
    StatisticalFeatureEngineer, AutomatedFeatureEngineer
)
from data.preprocessing import (
    PreprocessingPipeline, TimeSeriesScaler, CyclicalEncoder,
    LagFeatureCreator, OutlierDetector, FeatureSelector
)
...
class TestFeatureEngineering:
    ...
    def test_statistical_feature_engineer(self, sample_weather_data):
        ...
        enhanced_data = engineer.create_features(
            sample_weather_data,
            numeric_cols=['temperature', 'pressure', 'wind_speed']
        )
...
class TestPreprocessing:
    ...
    def test_time_series_scaler(self, sample_data):
        ...
        np.testing.assert_allclose(
            original_data, 
            sample_data[['numeric_1', 'numeric_2']].values, 
            rtol=1e-10
        )
