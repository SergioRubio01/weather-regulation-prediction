...
from config import ExperimentConfig, FNNConfig, LSTMConfig, RandomForestConfig
...
from training.trainer import DistributedTrainer, ExperimentTracker, ModelCheckpointer, Trainer
...
class TestTrainer:
    ...
    def test_cross_validation(self, sample_data, rf_model_config):
        ...
        cv_results = trainer.cross_validate(
            model=model, X=X, y=y, cv_folds=3, model_name="test_rf_cv"
        )
    ...
    def test_checkpoint_saving_loading(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            checkpoint_path = checkpointer.save_checkpoint(
                model_state=model_state, experiment_id="test_exp", model_name="test_model", epoch=10
            )
    ...
    def test_best_checkpoint_tracking(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            best_checkpoint = checkpointer.get_best_checkpoint(
                experiment_id="test_exp", model_name="test_model", metric_name="accuracy"
            )
...
class TestHyperparameterTuning:
    ...
    def rf_model_and_grid(self):
        ...
        model = RandomForestModel(config)
        param_grid = {"n_estimators": [5, 10], "max_depth": [2, 3, None]}
    ...
    def test_grid_search_tuner(self, sample_data, rf_model_and_grid):
        ...
        assert len(result.all_results) == len(param_grid["n_estimators"]) * len(
...
class TestExperimentRunner:
    ...
    def sample_config(self):
        ...
        config = ExperimentConfig(
            ...
            data=DataConfig(airports=["EGLL"], start_date="2023-01-01", end_date="2023-01-10"),
            training=TrainingConfig(
                test_size=0.2, random_state=42, cross_validation=True, cv_folds=3
            ),
            models={"random_forest": RandomForestConfig(n_estimators=5, random_state=42)},
        )
        return config
    ...
    def test_hyperparameter_tuning_experiment(self, mock_data_loader, sample_config, mock_data):
        ...
        results = runner.run_experiment_with_tuning(
            model_name="random_forest", param_grid=param_grid, tuning_method="grid", cv=2
        )
...
class TestPerformanceBenchmarks:
    ...
    def test_hyperparameter_tuning_benchmark(self, benchmark):
        ...
        config = RandomForestConfig(n_estimators=10, random_state=42)
        model = RandomForestModel(config)
        param_grid = {"n_estimators": [5, 10, 15], "max_depth": [2, 3, None]}
