...
class ConfigParser:
    ...
    def load_config(self, filepath: str | Path) -> ExperimentConfig:
        """
    ...
    def _parse_config_dict(self, config_dict: dict[str, Any]) -> ExperimentConfig:
        ...
        if "data" in config_dict:
            experiment_config.data = self._parse_dataclass(config_dict["data"], DataConfig)
        ...
        if "experiment_settings" in config_dict:
            ...
            experiment_config.hyperparameter_tuning = settings.get("hyperparameter_tuning", True)
        ...
        for field in fields(dataclass_type):
            if field.name in filtered_dict:
                ...
                if "datetime" in str(field.type) and isinstance(value, str):
                    filtered_dict[field.name] = datetime.strptime(value, "%Y-%m-%d %H:%M:%S")
    ...
    def merge_configs(
        ...
        override_config: ExperimentConfig | dict[str, Any],
    ) -> ExperimentConfig:
        """
    ...
    def generate_hyperparameter_grid(
        self, config: ExperimentConfig, model_name: str
    ) -> list[dict[str, Any]]:
        """
    ...
    def export_config(
        self, config: ExperimentConfig, filepath: str | Path, format: str = "yaml"
    ) -> None:
        """
    ...
    def validate_and_fix_config(
        ...
        validate_config(fixed_config)

        return fixed_config, fixes_applied

    def create_config_from_best_params(
        self, base_config: ExperimentConfig, model_name: str, best_params: dict[str, Any]
    ) -> ExperimentConfig:
        """
    ...
    def compare_configs(
        self, config1: ExperimentConfig, config2: ExperimentConfig
    ) -> dict[str, Any]:
        """
    ...
    def _find_differences(self, dict1: dict, dict2: dict, path: str = "") -> dict[str, Any]:
        ...
        for key in all_keys:
            ...
            if key not in dict1:
                ...
                nested_diff = self._find_differences(dict1[key], dict2[key], current_path)
                if nested_diff:
                    differences.update(nested_diff)
            elif dict1[key] != dict2[key]:
                differences[current_path] = {"config1": dict1[key], "config2": dict2[key]}

        return differences


# Utility functions for common operations
def load_config(filepath: str | Path) -> ExperimentConfig:
    """Load configuration from file."""
    parser = ConfigParser()
    return parser.load_config(filepath)


def save_config(config: ExperimentConfig, filepath: str | Path, format: str = "yaml"):
    """Save configuration to file."""
    parser = ConfigParser()
    parser.export_config(config, filepath, format)


def merge_configs(base: ExperimentConfig, override: ExperimentConfig | dict) -> ExperimentConfig:
