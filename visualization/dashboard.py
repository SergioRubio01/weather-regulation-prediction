...
class ModelComparisonDashboard:
    ...
    def create_layout(self):
        ...
        self.app.layout = dbc.Container([
            ...
            dbc.Row([
                dbc.Col([
                    html.H1("Weather Regulation Prediction - Model Comparison Dashboard",
    ...
    def setup_callbacks(self):
        ...
        def load_experiment_data(experiment_ids, metric):
            ...
            comparison_df = self.results_manager.compare_experiments(
    ...
    def _render_comparison(self, exp_ids):
        ...
        if not exp_ids or len(exp_ids) < 2:
            return html.Div("Please select at least 2 experiments to compare", 
        ...
        if experiments[0].best_model in experiments[0].model_results and \
           experiments[1].best_model in experiments[1].model_results:
            ...
            if cm1 is not None and cm2 is not None:
                ...
                fig_cm.add_trace(
                    go.Heatmap(z=cm1, colorscale='Blues', showscale=False),
                    row=1, col=1
                )
                fig_cm.add_trace(
                    go.Heatmap(z=cm2, colorscale='Blues', showscale=True),
                    row=1, col=2
                )
        ...
        return html.Div([
            html.H3("Detailed Experiment Comparison"),
            *comparison_items
        ])
            ...
            if exp:
                for model_name, model_result in exp.model_results.items():
                    stats_data.append({
                        ...
                        'Final Accuracy': f"{model_result.test_accuracy:.3f}" if model_result.test_accuracy else "N/A",
    ...
    def _render_weather(self, exp_ids):
        ...
        return html.Div([
            ...
            html.P("Weather pattern analysis and regulation correlation would be displayed here"),
            html.Hr(),
            dbc.Row([
                dbc.Col([
    ...
    def _render_reports(self, exp_ids):
        """Render reports tab"""
        if not exp_ids:
            return html.Div("Please select experiments to generate reports", 
                          className="text-center mt-5")
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        ...
                        options=[{'label': exp_id, 'value': exp_id} for exp_id in exp_ids],
                ...
                dbc.Col([
                    ...
                    dcc.Dropdown(
                        id='report-format',
                        options=[{'label': fmt.upper(), 'value': fmt} for fmt in export_formats],
                        value='excel'
                    )
                ], width=6)
            ]),
...
def create_dashboard(results_path: str = "./results", 
                    experiments: Optional[List[str]] = None) -> ModelComparisonDashboard:
