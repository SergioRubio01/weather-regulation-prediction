...
class ModelVisualizer:
    ...
    def plot_confusion_matrix(
        ...
        if interactive:
            ...
            fig = ff.create_annotated_heatmap(
                z=cm, x=labels, y=labels, annotation_text=text, colorscale="Blues", showscale=True
            )
    ...
    def plot_feature_importance(
        ...
        if interactive:
            ...
            fig.update_layout(
                title=title, height=600, width=400 * len(importance_data), template="plotly_white"
            )
    ...
    def plot_model_comparison(
        ...
        if interactive:
            ...
            for i, metric in enumerate(metrics):
                ...
                bars = ax.bar(
                    x + offset, comparison_df[metric], width, label=metric.replace("_", " ").title()
                )
        ...
        if interactive:
            fig = make_subplots(
                ...
                subplot_titles=[
                    ...
                    for title in (f"{model} - Class 0", f"{model} - Class 1")
                ],
                vertical_spacing=0.1,
            )
            ...
            for i, (model_name, probs) in enumerate(predictions.items()):
                # Class 0 predictions
                class_0_probs = (
                    probs[true_labels == 0][:, 1]
                    if len(probs.shape) > 1
                    else probs[true_labels == 0]
                )
                class_1_probs = (
                    probs[true_labels == 1][:, 1]
                    if len(probs.shape) > 1
                    else probs[true_labels == 1]
                )
            ...
            for i, (model_name, probs) in enumerate(predictions.items()):
                ...
                axes[i, 0].hist(
                    class_0_probs, bins=30, alpha=0.7, color="blue", label="True Class 0"
                )
                ...
                axes[i, 1].hist(
                    class_1_probs, bins=30, alpha=0.7, color="red", label="True Class 1"
                )
    ...
    def plot_learning_curves(
        ...
        if interactive:
            ...
            for i, model_name in enumerate(train_scores.keys()):
                ...
                ax.plot(train_sizes, train_mean, "o-", color=color, label=f"{model_name} - Train")
                ax.fill_between(
                    train_sizes,
                ...
                ax.plot(train_sizes, val_mean, "s--", color=color, label=f"{model_name} - Val")
                ax.fill_between(
                    train_sizes, val_mean - val_std, val_mean + val_std, alpha=0.2, color=color
                )
...
class WeatherVisualizer:
    ...
    def plot_regulation_analysis(
        ...
        fig = make_subplots(
            ...
            subplot_titles=[
                "Regulations by Hour",
                "Regulations by Weather Severity",
                "Weather Conditions During Regulations",
                "Regulation Duration Distribution",
            ],
            specs=[
        ...
        if "hour" in regulation_data.columns:
            ...
            fig.add_trace(
                go.Bar(x=hourly_regs.index, y=hourly_regs.values, name="Regulations"), row=1, col=1
            )
        ...
        for feature in weather_features:
            if feature in weather_data.columns and "has_regulation" in weather_data.columns:
                ...
                fig.add_trace(
                    go.Box(y=reg_weather, name=f"{feature} (Reg)", showlegend=False), row=2, col=1
                )
                fig.add_trace(
        ...
        if "duration" in regulation_data.columns:
            fig.add_trace(
                go.Histogram(x=regulation_data["duration"], nbinsx=30, name="Duration"),
                row=2,
                col=2,
            )

        fig.update_layout(
            title=title, height=800, width=1200, showlegend=False, template="plotly_white"
        )
    ...
    def plot_airport_comparison(
        ...
        for i, (airport, data) in enumerate(airport_data.items()):
            if metric in data.columns:
                fig.add_trace(
                    go.Scatter(
                        x=data.index, y=data[metric], mode="lines", name=airport, line=dict(width=2)
                    )
                )
...
def plot_model_results(results: dict[str, Any], save_path: str = "./visualizations"):
    ...
    if "training_history" in results:
        plots["training_history"] = visualizer.plot_training_history(
            results["training_history"], save_name=f"history_{results.get('experiment_id', 'exp')}"
        )
...
def create_experiment_dashboard(
    experiment_results: dict[str, Any], save_path: str = "./visualizations"
) -> str:
    ...
    # Save dashboard
    dashboard_path = (
        Path(save_path) / f"dashboard_{experiment_results.get('experiment_id', 'exp')}.html"
    )
